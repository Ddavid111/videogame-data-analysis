...
Ez a fejezet a korábbi fejezetekben előállított, egységesített \textit{merged} adathalmazra épül, amely a különböző forrásokból származó videójáték-metaadatok összefésült és normalizált nézetét tartalmazza. Az adat-előkészítési, összevonási és felbontási lépések eredményeként létrejött adatállomány lehetővé teszi a Steam \cite{steam} platformon megjelenő videójátékok jellemzőinek átfogó, statisztikai szemléletű vizsgálatát.
...
A Venn-diagram az \texttt{merge/generated/venn/venn\_diagram.png} állományba került mentésre, az elemszámokat tartalmazó összefoglaló táblázat pedig \texttt{venn\_table.csv} formátumban is elérhető.


\subsection{Forrásösszegzés (Source Summary)}

A \textit{merged master} táblában minden rekordhoz tartozik egy \textit{sources} mező, amely jelzi, hogy az adott sor mely eredeti adatforrás(ok)ból származik (A \cite{a}, B \cite{b} és C \cite{c}). Ez lehet egyetlen forrás (például \textit{A}), vagy több forrás kombinációja (például \textit{B, C} vagy \textit{A,B,C}).
...
Az összesítés eredménye egy CSV formátumú táblázatban kerül mentésre, amely a \texttt{merge/generated/reports/source\_summary.csv} fájlban érhető el. Ez a táblázat jól áttekinthető módon mutatja meg az egyes forráskombinációkhoz tartozó rekordok számát, valamint azt is, hogy az adott kombináció tartalmazza-e az A, B vagy C forrást.
...
Az ellenőrzés eredménye egy külön CSV jelentésben kerül mentésre \\ \texttt{merge/generated/reports/integrity\_report.csv} néven, amely a hibák számát ellenőrzésenként összesítve tartalmazza. Ez a jelentés az összefésülés-folyamat utólagos ellenőrzésére és dokumentálására is alkalmas.
...
A diagramok a \texttt{notebooks/merge/generated/plots} könyvtárban érthetők el.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_pre2010.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_post2010.png}
...
\begin{itemize}
    \item \textbf{A adathalmaz \cite{a}} – Steam \cite{steam} metaadatok korábbi, strukturált CSV-alapú forrása
    \item \textbf{B adathalmaz \cite{b}} – SteamSpy \cite{steamspy} adatok (JSON-alapú, külső statisztikai forrás)
    \item \textbf{C adathalmaz \cite{c}} – 2024--2025-ös, legfrissebb Steam adatgyűjtések
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_sources_A.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_sources_B.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_sources_C.png}
...
A \ref{fig:top5} ábra a Steam \cite{steam} platformon megjelent videójátékok közül az
öt leggyakoribb műfaj évenkénti megjelenésszámát mutatja be az adatbázis teljes időtartamára
(1997--2025 májusáig).

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_genres_top5.png}
    \caption{A Top 5 műfaj időbeli trendje}
    \label{fig:top5}
\end{figure}

A legnépszerűbb műfajok a vizsgált időszakban az alábbiak voltak:
\textit{Indie}, \textit{Casual}, \textit{Action}, \textit{Adventure}, \textit{Simulation}.
...
A \ref{fig:hist_growth_rates} ábra az egyes évek közötti növekedési faktort mutatja be,
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/hist_growth_rates.png}
...
A \textbf{2015} utáni időszakban a növekedési faktor stabilizálódása figyelhető meg,
...
A \ref{fig:heatmap_year_day} hőtérkép az egyes évek és naptári napok mentén mutatja a videójáték \\
 megjelenések intenzitását. A színezés a napi publikálások számát reprezentálja,
így egyszerre szemlélteti a hosszú távú növekedési trendet
és az esetleges szezonális mintázatokat.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.2\textwidth]{images/heatmap_year_day.png}
...
A \ref{fig:heatmap_year_weekday_aligned} ábra a videójáték-megjelenések
...
azon napok, amelyek az év különböző időpontjaiban a hétnek ugyanarra a napjaira estek.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.2\textwidth]{images/heatmap_year_weekday_aligned.png}
    \caption{Év $\times$ (ISO hét $\times$ hét napja) hőtérkép a játékmegjelenések intenzitásáról}
    \label{fig:heatmap_year_weekday_aligned}
\end{figure}
...
hatását (például azt, hogy ugyanaz a dátum különböző években a hét más napjaira esik), és lehetővé teszi a hét napjaihoz kötődő ciklikus
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/seasonal_monthly_overlay.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/seasonal_weekly_overlay.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/seasonal_weekly_overlay_iso.png}
...
A trendillesztés során az éves játékmegjelenések számát
természetes alapú logaritmikus transzformációnak vetettem alá,
majd az így kapott adatokra lineáris regressziós modellt illesztettem
legkisebb négyzetek módszerével.
Az illesztés Python környezetben, a \texttt{scikit-learn} könyvtár
\texttt{LinearRegression} osztályának felhasználásával történt,
alapértelmezett beállítások mellett.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/forecast_2026_log_modern.png}
...
A modell becslési pontosságát visszatesztelés során az abszolút
százalékos hiba (APE, Absolute Percentage Error) \cite{APE} segítségével
értékeltem, amely az alábbi módon számítható:
\begin{equation}
\mathrm{APE} = \frac{|\hat{y} - y|}{y} \cdot 100\%,
\end{equation}
ahol $y$ a tényleges megfigyelt érték, $\hat{y}$ pedig a modell által
becsült érték.
...
expanding-window \cite{expanding_window} típusú, egy-lépéses visszateszteléssel vizsgáltam.
...
majd a trendillesztéshez felhasznált megfigyelési pontok száma
évről-évre bővül, ahol egy tréningpont egy adott év
összesített játékmegjelenésszámát jelenti. A becslési hiba 
alakulását az abszolút százalékos hiba (APE) segítségével értékeltem.

Az eredményeket a \ref{fig:expanding_backtest_log} ábra szemlélteti.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/expanding_backtest_ape_log_modern.png}
    \caption{Expanding-window visszatesztelés – a becslési hiba alakulása a trendillesztéshez felhasznált tréningévek számának függvényében}
...
a becslési hiba fokozatosan csökken, majd stabilizálódik.
A 10 éves tréningablaknál a relatív hiba már megközelítőleg 10\%,
ami a modell konzisztens viselkedését jelzi.

Az abszolút hiba alakulását a becsült és a tényleges értékek különbségeként
a \ref{fig:expanding_backtest_error} ábra mutatja be.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/expanding_backtest_error_log_modern.png}
    \caption{Expanding-window visszatesztelés – előrejelzési hiba alakulása}
    \label{fig:expanding_backtest_error}
\end{figure}

Az ábra alapján megfigyelhető, hogy a vizsgált időszakban a becslési hiba
jellemzően pozitív, ami arra utal, hogy a modell többnyire felülbecsüli
a tényleges játékmegjelenésszámot. A hiba nagysága ugyanakkor a
tréningidőszak bővülésével mérséklődik.

A becsült és a tényleges értékek időbeli együttalakulását
a \ref{fig:expanding_backtest_pred_vs_actual} ábra szemlélteti.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/expanding_backtest_pred_vs_actual_log_modern.png}
    \caption{Expanding-window visszatesztelés – becsült és tényleges értékek összehasonlítása}
    \label{fig:expanding_backtest_pred_vs_actual}
\end{figure}

Az ábra alapján látható, hogy a becslések jól követik a játékmegjelenések
növekvő trendjét, azonban a becsült értékek több évben a tényleges adatok
felett helyezkednek el, ami a modell optimista jellegére utal.
...
nem tisztán exponenciális jellegű,
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\textwidth]{images/forecast_2024_log_modern.png}
    \caption{2024-es előrejelzés tesztje a log-lineáris modell alapján a korábbi adatokra}
    \label{fig:forecast_2024}
\end{figure}

A 2024-es évre vonatkozó teszt eredményei a \ref{fig:forecast_2024} ábrán láthatók.
A modell teljesítményének ellenőrzésére a 2014–2023 közötti adatokon
tanított log-lineáris (legkisebb négyzetek módszerével illesztett)
trend segítségével előrejeleztem a 2024-es évet.
