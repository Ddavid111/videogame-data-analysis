...
def create_requirements_table(master_df: pd.DataFrame,
    ...
    for _, r in master_df.iterrows():
        ...
        if pd.notna(pc_val) and str(pc_val).strip():
            try:
                ...
                win_min = clean_requirements_text(
                    val_dict.get('minimum', "")
                )
                win_rec = clean_requirements_text(
                    val_dict.get('recommended', "")
                )
            except Exception:
    ...
    if output_dir:
        ...
        logging.info(
            f"Saved 'requirements.csv' ({len(df_req)} rows) to {output_dir}"
        )
