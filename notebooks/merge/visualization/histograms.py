...
MERGED_PATH = os.path.join(NOTEBOOKS_ROOT, "merge", "generated", "tables", "merged_master.csv")
OUTPUT_DIR = os.path.join(NOTEBOOKS_ROOT, "merge", "generated", "plots")
...
def plot_heatmap_year_day(df, output_dir):
    ...
    cbar = plt.colorbar(label="Megjelenések száma")
    cbar.ax.tick_params(labelsize=13)
    cbar.set_label("Megjelenések száma", fontsize=15)
    plt.title("Játékmegjelenések hőtérképe – év × nap", fontsize=20)
    plt.xlabel("Nap (1–365)", fontsize=16)
    plt.ylabel("Év", fontsize=16)

    plt.xticks(fontsize=13)
    plt.yticks(ticks=np.arange(len(heat.index)), labels=heat.index, fontsize=13)
...
def plot_heatmap_year_weekday_aligned(df, output_dir):
    ...
    cbar.ax.tick_params(labelsize=13)
    cbar.set_label("Megjelenések száma", fontsize=15)
    plt.title("Játékmegjelenések hőtérképe – év × (ISO hét × 7 + hét napja)", fontsize=20)
    plt.xlabel("Idő (hetek 1–53, naponként bontva)", fontsize=16)
    plt.ylabel("Év", fontsize=16)
    plt.xticks(fontsize=13)
    plt.yticks(ticks=np.arange(len(heat.index)), labels=heat.index, fontsize=13)
